<script lang="ts">
    ...
    async function createRoom(e): Promise<void> {
    ...
    function updateCurrentUser(user): void {
        currentUser = user;
    ...
    function findMyChannels(rooms): string[] {
        let myChannels: any = [];
    ...
    function findCurrentChat(rooms: any): void {
    ...
    function findCurrentPrivateMessages(privateMessages: any): void {
    ...
    function initAll(init: any): void {
        ...
        myChannels = findMyChannels(rooms);
        findCurrentChat(rooms);
        findCurrentPrivateMessages(privateMessages);
    }

    function createChannel(channel: any): void {
    ...
    function deletePrivateMessage(room: any): void {
    ...
    function leaveRoom(room: any): void {
    ...
    function changeConv(title: any): void {
    ...
    function changeConvMessages(title: any): void {
    ...
    function receivedMessage(message: any): void {
    ...
    function updateChannels(update: any): void {
        let allUpdate = [];
    ...
    async function createPrivateMessage(): Promise<void> {
        await socket.emit('createPrivateMessage', currentUser.userName42);
    }

    onMount(async (): Promise<void> => {
        if ($logged == 'true') {
            let block = await fetch(`${BACKEND_URL}/users/${$id}`, {
                method: 'GET',
                credentials: 'include',
                headers: {
            ...
            <div class="header">
                ...
                {#if currentRoom}
                    {#if currentRoom.isDirectMessage != true}
                        <h3 class="roomTitle">Room #{currentRoom.name.toUpperCase()}</h3>
                    {:else}
                        <h3 class="roomTitle">
                    ...
                    {/if}
                {:else}
                    <h3 style="font-style: italic;" class="roomTitle">
                    ...
                    <div id="messages">
                        {#if currentRoom}
                            ...
                            {#if myChannels.indexOf(currentRoom.name) == -1 && currentRoom.isPublic == true && currentRoom.isDirectMessage != true}
                                <button
                                        class="joinButton"
                                        on:click|preventDefault={() => joinRoom()}>Join room
                                </button
                                >
                      ...
                      banOptions = 'false';
                                ...
                                {#if muteOptions == 'true'}
                                    <div>
                                        ...
                                        <button on:click|preventDefault={muteUser}>Mute User
                                        </button
                                        >
                                    </div>
                                {/if}
                      ...
                      muteOptions = 'false';
                            ...
                            {#if currentRoom.channelOwnerId == $id && currentRoom.channelAdminsId.indexOf(currentUser.id.toString()) == -1}
                                <button
                                        ...
                                        class="profileButton">Upgrade status
                                </button
                                >
                            {:else if currentRoom.channelOwnerId == $id && currentRoom.channelAdminsId.indexOf(currentUser.id.toString()) != -1}
                                <button
                                        ...
                                        class="profileButton">Downgrade status
                                </button
                                >
                            {/if}
                        {/if}
