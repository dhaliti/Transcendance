<script lang="ts">
    ...
    function cancelGameInvitation(): void {
        ingame = 'false';
    ...
    function declinedResponse(): void {
    ...
    async function gameRequest(): Promise<void> {
        ingame = 'waiting';
        socket.emit('waiting');
    }
    ...
    async function watchGame(game): Promise<void> {
        currentGame = game;
    ...
    onMount(async (): Promise<void> => {
        ...
        if ($logged == 'true') {
            ...
            socket.on('watchGameResponse', async (message): Promise<void> => {
                if (message == 'noGame') {
            ...
            socket.on('forfeitedGame', async (): Promise<void> => {
                playing = false;
                ingame = 'false';
                context.clearRect(0, 0, width, height);
    ...
    {#if ingame == 'true'}
        <div style="display: flex; margin: 0 auto; width: 400px;">
            ...
            {:else}
                ...
                >
            {/if}
            <button on:click|preventDefault={forfeit} class="forfeit_button">Forfeit the game
            </button
            >
        </div>
        <div class="ingame"
